...
class User(db.Model):
    ...
    orders = db.relationship("Order", backref="account", cascade="all, delete")
    ...
    def registrations_per_day():
        stmt = text("SELECT COUNT(date_created) AS registrations, DATE(date_created) AS date FROM account GROUP BY DATE(account.date_created)")
